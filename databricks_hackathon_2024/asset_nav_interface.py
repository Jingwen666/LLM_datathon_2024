...
# MAGIC %run /Workspace/Repos/jingwen_huang@transalta.com@gmail.com/databricks_hackathon_2024/_resources/00-init-advanced $reset_all_data=false
# MAGIC
...
### customized chatbot interface ###
# Import the required libraries


# Define the function to display the chat history

def display_chat(chat_history):
    chat_history_html = "<div style='border: 1px solid #ccc; padding: 10px; max-height: 300px; overflow-y: scroll;'>"
    # Reverse the chat history to display the newest conversation first
    chat_history = reversed(chat_history)
    for message in chat_history:
        if message["role"] == "user":
            # Display the user's message on the right side of the chat window
            chat_history_html += "<div style='display: flex; justify-content: flex-end;'><div style='background-color: #f2f2f2; padding: 5px; border-radius: 10px; margin-bottom: 10px;'>{}<img src='https://dbc-09a2e2f2-c253.cloud.databricks.com/?o=2747756931234785#files/245734104242948' style='vertical-align:middle; width:20px; height:20px;'></div></div>".format(message["content"])
        else:
            # Display the chatbot's message on the left side of the chat window
            chat_history_html += "<div style='display: flex;'><div style='background-color: #e6e6e6; padding: 5px; border-radius: 10px; margin-bottom: 10px;'><img src='/Workspace/Users/jingwen_huang@transalta.com/databricks_hackathon_2024_jw_test/thumbnail_chatbot1.png' style='vertical-align:middle; width:20px; height:20px;'>{}</div></div>".format(message["content"])
    chat_history_html += "</div>"
    display(HTML(chat_history_html))
# Define the function to invoke the chatbot model and get the response
...
def get_chatbot_response(user_input, dialog_history_for_rag, dialog_history_for_user):
    dialog_history_for_rag["messages"].append({"role": "user", "content": user_input})
    dialog_history_for_user["messages"].append({"role": "user", "content": user_input})
...
hbox_layout = widgets.Layout(display='flex', flex_flow='row', align_items='flex-start')

# Create the user input widget and set its style
user_input_widget = widgets.Text(description='User Question:', style={'description_width': 'initial'})

# Create the submit button and set its style
submit_button = widgets.Button(description='Submit', style={'button_color': '#4caf50'})

# Create the HBox container and add the widgets to it
hbox_container = widgets.HBox([user_input_widget, submit_button], layout=hbox_layout)

loading_spinner = widgets.IntProgress(value=0, min=0, max=100, description='Loading:', style={'bar_color': 'blue'}, layout=widgets.Layout(width='50%'))
...
def submit_button_clicked(b):
    ...
    # Call the get_chatbot_response function with the user input
    dialog_history_for_user_messages = get_chatbot_response(user_input, dialog_history_for_rag, dialog_history_for_user)
    
    # Clear the output widget and display the chat history with proper styling
    with output_widget:
        output_widget.clear_output()
        display_chat(dialog_history_for_user_messages)
...
# Display the widgets
display(hbox_container, output_widget)
